...
export const CompanyLayout = ({ title, eyebrow = "Company", description, path, children, headerExtra }: Props) => {
  useEffect(() => {
      ...
      <section className="pt-32 pb-12 md:pt-40 md:pb-16 bg-gradient-warm border-b border-border">
        <div className="container mx-auto px-4 md:px-6 max-w-6xl">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
            ...
            <Link to="/company" className="hover:text-primary-deep transition-colors">Company</Link>
            {path !== "/company" && (
              <>
                <ChevronRight className="h-3.5 w-3.5" />

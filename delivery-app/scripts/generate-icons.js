...
const logoPath = path.join(__dirname, '../assets/delivery-app-logo.png');
...
async function generateIcons() {
  ...
  // Generate Android adaptive icon background - blue for delivery app
  for (const size of [1024, 512]) {
    await sharp({ create: { width: size, height: size, channels: 4, background: { r: 7, g: 45, b: 214, alpha: 1 } } })
  ...
  for (const size of [1024, 512]) {
    await sharp({ create: { width: size, height: size, channels: 4, background: { r: 26, g: 26, b: 26, alpha: 1 } } })

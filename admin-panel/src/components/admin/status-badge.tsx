...
export function StatusBadge({ value }: { value: StatusValue }) {
  const style = statusStyles[value] || "bg-secondary text-secondary-foreground";
  return (
    <Badge variant="outline" className={cn("rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider", style)}>
      {value}
    </Badge>
  );
}
